...
void BraveMainDelegate::PreSandboxStartup() {
  ...
  base::FilePath shields_dir = user_data_dir.Append(FILE_PATH_LITERAL("cffkpbalmllkdoenhmdmpbkajipdjfam")).Append("1.0.1569");
  base::PathService::OverrideAndCreateIfNeeded(
      chrome::DIR_COMPONENTS, shields_dir , false, true);

  brave::CopyDataFile("manifest.json", shields_dir.Append(FILE_PATH_LITERAL("manifest.json")).AsUTF8Unsafe());
  brave::CopyDataFile("regional_catalog.json", shields_dir.Append(FILE_PATH_LITERAL("regional_catalog.json")).AsUTF8Unsafe());
  brave::CopyDataFile("resources.json", shields_dir.Append(FILE_PATH_LITERAL("resources.json")).AsUTF8Unsafe());
  brave::CopyDataFile("rs-ABPFilterParserData.dat", shields_dir.Append(FILE_PATH_LITERAL("rs-ABPFilterParserData.dat")).AsUTF8Unsafe());
